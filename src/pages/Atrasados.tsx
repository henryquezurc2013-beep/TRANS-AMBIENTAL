    ...
    <div className="page-container">
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          <AlertTriangle size={40} color="hsl(142,71%,45%)" style={{ margin: '0 auto 1rem' }} />
          <h3 style={{ margin: '0 0 0.5rem', fontWeight: 600 }}>Tudo em dia!</h3>
          <p style={{ margin: 0, color: 'hsl(210,20%,50%)', fontSize: '0.875rem' }}>Nenhum container com retirada atrasada.</p>
        </div>
      ) : (
          ...
          {atrasados.map(c => {
            ...
            return (
              <div key={c.id} style={{
                ...
                background: estaEditando ? 'hsl(217 91% 60% / 0.05)' : `${cor}08`,
                border: `1px solid ${estaEditando ? 'hsl(217 91% 60% / 0.3)' : `${cor}25`}`,
                borderRadius: '0.75rem',
                borderLeft: `3px solid ${estaEditando ? 'hsl(217,91%,60%)' : cor}`,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '0.8rem', color: 'hsl(210,20%,55%)', display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <span>Entregue em {c.data_entrega.split('-').reverse().join('/')}</span>
                    <span>·</span>

                    {/* Previsão — modo edição ou visualização */}
                    {estaEditando ? (
                      ...
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                        ...
                        <button
                          className="btn-ghost"
                          style={{ padding: '0.125rem 0.3rem', lineHeight: 1 }}
                          onClick={() => iniciarEdicao(c.id, c.previsao_retirada)}
                          title="Editar previsão de retirada"
                        >
                          <Pencil size={11} />
                        </button>
                      </span>
                ...
                {!estaEditando && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: '0.7rem', color: 'hsl(210,20%,50%)' }}>dias</div>
                    </div>
                    <button
